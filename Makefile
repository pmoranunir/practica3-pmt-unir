...
test-unit:
	docker run --name unit-tests --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest /bin/bash -c "pytest --cov --cov-report=xml:results/coverage.xml --cov-report=html:results/coverage --junit-xml=results/unit_result.xml -m unit; junit2html results/unit_result.xml results/unit_result.html" || true
...
test-api:
	...
	docker run --network calc-test-api --name api-tests --env PYTHONPATH=/opt/calc --env BASE_URL=http://apiserver:5000/ -w /opt/calc calculator-app:latest /bin/bash -c "pytest --junit-xml=results/api_result.xml -m api; junit2html results/api_result.xml results/api_result.html" || true
...
test-e2e:
	...
	docker run -it -d --name converter --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest bash || true
	docker exec converter /bin/bash -c "mkdir -p /opt/calc/results" || true
	docker cp ./results/cypress_result.xml converter:/opt/calc/results/ || true
	docker exec converter /bin/bash -c "junit2html results/cypress_result.xml results/cypress_result.html" || true
	docker cp converter:/opt/calc/results/cypress_result.html ./results/ || true
	docker stop converter || true
	docker rm --force converter || true	
	docker rm --force apiserver  || true
	docker rm --force calc-web || true
	docker rm --force e2e-tests || true
